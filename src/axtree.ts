...
export async function getFullAXTree(urlOrPath: string, options: GetAXOptions = {}) {
    const {
        waitForMs = 1000,
        waitForSelector,
        waitForStableMs = 500,
...
// Exported helper to generate AX tree from a raw HTML string
export async function getFullAXTreeFromHtml(html: string, options: GetAXOptions = {}) {
    const {
        waitForMs = 1000,
        waitForSelector,
        waitForStableMs = 500,
    ...
    const page = await context.newPage()
    try {
        // Load the provided HTML string into the page
        // Comments in code are in English as per repo instructions
